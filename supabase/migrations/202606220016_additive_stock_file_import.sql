...
create table if not exists public.stock_import_entries (
  ...
  unique (user_id, batch_id, row_key)
);

create index if not exists stock_import_entries_user_batch_idx
  on public.stock_import_entries(user_id, batch_id, created_at);

alter table public.stock_import_entries enable row level security;

revoke all on public.stock_import_entries from public, anon;
grant select on public.stock_import_entries to authenticated;

drop policy if exists "Users can read own stock import entries" on public.stock_import_entries;
create policy "Users can read own stock import entries"
on public.stock_import_entries
for select to authenticated
using (auth.uid() = user_id);

revoke insert, update, delete on public.stock_import_entries from authenticated;
  ...
  if p_product_id is not null then
    ...
    if not found then
      insert into public.products (
        ...
        category, product_type, current_stock, reserved_stock, min_stock, critical_stock,
        ...
        case when p_product_type = 'main_product' then 'Ana Ürün' else 'Toplu Yükleme' end,
        case when p_product_type = 'main_product' then 'main_product' else 'component' end,
        0, 0, 0, 0, 'Toplu dosya stok girişi',
        'Excel/PDF stok girişi sırasında otomatik oluşturuldu.', now(), now(), null
      )
...
comment on function public.apply_stock_increment_import(uuid, text, text, numeric, text, text, uuid, text, text, text) is
  'Excel/PDF stok satirini tenant urunune atomik ve idempotent olarak ekler; kod yoksa yeni kart acmaz.';
