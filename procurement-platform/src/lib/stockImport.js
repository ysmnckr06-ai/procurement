...
export const STOCK_IMPORT_FIELDS = [
  ...
  { key: "criticalStock", label: "Kritik stok", required: false },
];
...
const CURRENCY_ALIASES = {
  TRY: ["try", "tl", "₺", "turk lirasi", "türk lirası"],
...
function aliasScore(value, alias) {
  ...
  if (value.includes(alias) || alias.includes(value))
    return Math.min(value.length, alias.length) >= 3 ? 0.82 : 0;
  const valueTokens = new Set(value.split(" "));
  const aliasTokens = alias.split(" ");
  const overlap = aliasTokens.filter((token) => valueTokens.has(token)).length;
  return overlap
    ? 0.55 + (0.2 * overlap) / Math.max(valueTokens.size, aliasTokens.length)
    : 0;
}
...
function headerFieldScore(value, field) {
  ...
  return Math.max(
    0,
    ...COLUMN_ALIASES[field].map((alias) =>
      aliasScore(normalized, normalizeImportHeader(alias)),
    ),
  );
}
...
export function detectHeaderRow(matrix) {
  ...
  rows.slice(0, 20).forEach((row, index) => {
    const fieldScores = Object.keys(COLUMN_ALIASES).map((field) =>
      Math.max(...row.map((cell) => headerFieldScore(cell, field))),
    );
    ...
    if (
      ...
      (fieldCount === best.fieldCount && score > best.score)
    )
      best = { index, score, fieldCount };
  });
  return best.fieldCount >= 2
    ? best
    : { index: -1, score: best.score, fieldCount: best.fieldCount };
}
...
export function parseStockNumber(value) {
  ...
  if (
    /[a-zA-ZÇĞİÖŞÜçğıöşü]/.test(original) &&
    !/^[-+]?\s*\d[\d.,\s]*(adet|ad|pcs|pc|kg|gr|g|mt|m|cm|mm|lt|l|try|tl|usd|eur)?$/i.test(
      original,
    )
  )
    return null;
...
export function normalizeImportCurrency(value) {
  ...
  for (const [currency, aliases] of Object.entries(CURRENCY_ALIASES)) {
    if (aliases.some((alias) => normalized === normalizeImportHeader(alias)))
      return currency;
  }
  const upper = raw.toUpperCase();
  return ["TRY", "USD", "EUR", "GBP"].includes(upper) ? upper : "";
}
...
function columnStats(values) {
  ...
  const units = present.filter((value) =>
    UNIT_VALUES.has(normalizeImportHeader(value)),
  );
  const currencies = present.filter((value) => normalizeImportCurrency(value));
  const codeLike = present.filter(
    (value) =>
      value.length <= 28 &&
      /[a-zA-Z]/.test(value) &&
      /[0-9]/.test(value) &&
  ...
  const alpha = present.filter(
    (value) =>
      /[a-zA-ZÇĞİÖŞÜçğıöşü]/.test(value) && parseStockNumber(value) === null,
  );
  const averageLength = present.length
    ? present.reduce((sum, value) => sum + value.length, 0) / present.length
    : 0;
  ...
  return {
    ...
    currencyRatio: currencies.length / denominator,
    codeRatio: codeLike.length / denominator,
    alphaRatio: alpha.length / denominator,
    averageLength,
    decimalRatio:
      numeric.filter((value) => /[.,]\d{2}$/.test(value)).length / denominator,
  };
}
...
function chooseUnique(candidates, used, minimum = 0.45) {
  const candidate = candidates
    ...
    .find((item) => !used.has(item.index) && item.score >= minimum);
...
export function inferStockColumns(matrix, options = {}) {
  ...
  const samplesByColumn = Array.from({ length: width }, (_, index) =>
    rows.slice(dataStartIndex, dataStartIndex + 100).map((row) => row[index]),
  );
  ...
  for (const field of Object.keys(COLUMN_ALIASES)) {
    const headerCandidates = headers.map((header, index) => ({
      index,
      score: headerFieldScore(header, field),
    }));
  ...
  const contentScores = {
    ...
    productCode: stats.map((stat, index) => ({
      index,
      score:
        stat.codeRatio * 0.9 +
        (stat.averageLength > 2 && stat.averageLength < 24 ? 0.1 : 0),
    })),
    productName: stats.map((stat, index) => ({
      index,
      score: stat.alphaRatio * 0.55 + Math.min(stat.averageLength / 45, 0.45),
    })),
    quantity: stats.map((stat, index) => ({
      index,
      score: stat.numericRatio * 0.9 + (stat.decimalRatio < 0.5 ? 0.1 : 0),
    })),
    unitPrice: stats.map((stat, index) => ({
      index,
      score: stat.numericRatio * 0.65 + stat.decimalRatio * 0.35,
    })),
    currency: stats.map((stat, index) => ({
      index,
      score: stat.currencyRatio,
    })),
    criticalStock: stats.map((stat, index) => ({
      index,
      score: stat.numericRatio * 0.85 + (stat.decimalRatio < 0.4 ? 0.15 : 0),
    })),
    brand: stats.map((stat, index) => ({
      index,
      score:
        stat.alphaRatio * 0.55 +
        (stat.averageLength >= 2 && stat.averageLength <= 20 ? 0.2 : 0),
    })),
  };
  ...
  for (const field of [
    ...
    const minimum =
      field === "brand" || field === "unitPrice" || field === "criticalStock"
        ? 0.66
        : 0.48;
...
export function buildStockImportRows(
  analysis,
  mapping,
  requiresQuantity,
  fileName,
      ...
      return [
        {
          ...
          productCode:
            mapping.productCode !== null
              ? String(source[mapping.productCode] ?? "").trim()
              : "",
          productName,
          brand:
            mapping.brand !== null
              ? String(source[mapping.brand] ?? "").trim()
              : "",
          unit:
            mapping.unit !== null
              ? String(source[mapping.unit] ?? "").trim() || "adet"
              : "adet",
          quantity,
          unitPrice:
            mapping.unitPrice !== null
              ? parseStockNumber(source[mapping.unitPrice])
              : null,
          currency:
          ...
          criticalStock:
            mapping.criticalStock !== null
              ? parseStockNumber(source[mapping.criticalStock])
              : null,
        },
      ];
    });
...
export function analyzeStockMatrix(matrix, options = {}) {
  ...
  Object.keys(options.mapping || {}).forEach((field) => {
    confidence[field] = 1;
  });
  ...
  const weakFields = Object.entries(mapping)
    ...
    .map(
      ([field]) =>
        STOCK_IMPORT_FIELDS.find((item) => item.key === field)?.label || field,
    );
  const parsedRows = buildStockImportRows(
    inferred,
    mapping,
    requiresQuantity,
    options.fileName,
  ...
  const requiredScores = [
    ...
    ...(requiresQuantity ? [confidence.quantity || 0] : []),
  ];
  const overallConfidence = requiredScores.length
    ? Math.round(
        (requiredScores.reduce((sum, score) => sum + score, 0) /
...
export function mapStockImportColumns(headers) {
  ...
  return Object.fromEntries(
    Object.entries(analysis.mapping).map(([field, index]) => [
      field,
...
export function parseStockImportRows(
  ...
  const indexMapping = Object.fromEntries(
    Object.entries(mapping).map(([field, header]) => [
      field,
      header === null ? null : headers.indexOf(header),
    ]),
  );
  const analysis = inferStockColumns(matrix, { headerRowIndex: 0 });
  return buildStockImportRows(
    analysis,
    indexMapping,
    requiresQuantity,
    fileName,
